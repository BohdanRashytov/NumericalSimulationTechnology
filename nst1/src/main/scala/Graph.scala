...
object Graph {
  ...
  def paintLines(f: (Double, Double) => (Double, Double), ps: List[(Double, Double)], defaultPoint: List[(Double, Double)], plotName: String) = {
    val points = ps.filterNot(pp => pp._1 == 2.0 && pp._1 >= 1.0 && pp._1 <= 2.0)
    ...
    val lines = points.map(point => Array(Array(point._1, point._2), {
      val alpha = Math.atan(f(point._1, point._2)._1 / f(point._1, point._2)._2)
      val xx = point._1 + Math.sin(alpha)*r(point._1, point._2)*0.025
      val yy = point._2 + Math.cos(alpha)*r(point._1, point._2)*0.025
      Array(xx, yy)
    }))
  ...
  def paintPlot(f: (Double, Double) => Double, points: List[(Double, Double)], defaultPoint: List[(Double, Double)], plotName: String, ff: Option[(Double, Double) => (Double, Double)]) = {
