...
object Graph {
  ...
  def paintLines(f: (Double, Double) => (Double, Double), ps: List[(Double, Double)], defaultPoint: List[(Double, Double)], vihPoint: List[(Double, Double)], plotName: String) = {
    ...
    val lines = points.map(point => Array(Array(point._1, point._2), {
      val xx = point._1 + f(point._1, point._2)._1 / d(point, f(point._1, point._2))*0.01
      val yy = point._2 + f(point._1, point._2)._2 / d(point, f(point._1, point._2))*0.01
      Array(xx, yy)
    }))
  ...
  def d(p1: (Double, Double), p2: (Double, Double)) = Math.sqrt((p1._1 - p2._1)*(p1._1 - p2._1) + (p1._2 - p2._2)*(p1._2 - p2._2))
  ...
  def paintPlot(f: (Double, Double) => Double, points: List[(Double, Double)], defaultPoint: List[(Double, Double)], vihPoint: List[(Double, Double)], plotName: String, ff: Option[(Double, Double) => (Double, Double)]) = {
    ...
    if (!ff.isEmpty) {
      ...
      val mmax = 1.0
      val mmin = min + qw
      val delta = (mmax - mmin) / countOfColors

      (1 to countOfColors).foreach(i => {
        val ymin = mmin + (i - 1)*delta
        val ymax = mmin + i*delta
        val colorPoint = (for {
          k <- 1 until K
          l <- 1 until L
        } yield {
